...
public class EnumBusterReflect {
    ...
    public Enum<?> make(
        ...
        try {
            ...
            Constructor ca = findConstructor(additionalTypes, clazz);
    ...
    private boolean tryReplaceExisting(
        ...
        for(int i = 0; i < values.length; i++) {
            ...
            if(value.name()
                .equals(next.name())) {
                setOrdinal(next, value.ordinal());
                values[i] = next;
                replaceConstant(next);
                return true;
            }
        }
    ...
    public void addByValue(Enum<?> e) {
        try {
            ...
            Enum<?>[] values = values();
            if(tryReplaceExisting(e, values)) return;

            appendToValues(e, values, valuesField);
        } catch(Exception ex) {
    ...
    private Constructor findConstructor(
        ...
        Class[] actualTypes = new Class[types.length + 2];
        actualTypes[0] = String.class;
        actualTypes[1] = int.class;
        System.arraycopy(types, 0, actualTypes, 2, types.length);

        return clazz.getDeclaredConstructor(actualTypes);
    }
    ...
    private Collection<Field> findRelatedSwitchFields()
        ...
        for(ClassInfo classInfo : foundClasses) {
            for(FieldInfo field : classInfo.getFields()) {
                String switchMapName = "$SwitchMap$" +
                                       clazz.getName()
                                           .replace('.', '$');
                if(field.getName()
                    .equals(switchMapName)) {
                    count++;
                    if(Loader.DEBUG) System.out.println("  " + classInfo.getClassName());
                    Field realField = loader.loadClass(classInfo.getClassName())
    ...
    @SuppressWarnings("unchecked") private Enum<?>[] values()
        throws NoSuchFieldException, IllegalAccessException {
        Field valuesField = findValuesField();

...
public class WordWrapWriter extends PrintWriter {
    ...
    public synchronized void write(int c) {
        if(buffer == null) buffer = new StringBuffer(lineLength * 2);

        buffer.append((char) c);
        if(c == NEWLINE_MARKER) flush();
    }
    ...
    private void flushBuffer(StringBuffer buf) {
        ...
        StringTokenizer lineTok = new StringTokenizer(buf.toString(), "" + NEWLINE_MARKER, true);
        ...
        if(currentLength > 0) writer.println();
